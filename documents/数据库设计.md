...
**旧方案**：前端使用 `Date.now().toString()` 或 `pdf_${Date.now()}` 生成 chatId，然后传给后端。

**新方案**：
1. 会话ID由后端数据库自增生成（`chat_session.id` 使用 `bigserial` 自增主键）
2. 前端在需要创建会话时，调用 `/api/v1/ai/session/create` 接口
3. 后端返回生成的会话ID给前端
4. 前端使用返回的会话ID进行后续操作
...
参数：
  - userId: Long (必填) 用户ID
  - type: String (必填) 会话类型 (chat/pdf/game/service)
